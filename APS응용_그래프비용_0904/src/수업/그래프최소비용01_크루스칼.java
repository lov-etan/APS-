...
public class 그래프최소비용01_크루스칼 {
	static class Edge implements Comparable<Edge>{
		...
		public int compareTo(Edge o) {
			...
			return this.W - o.W;
			//내림차순 하고 싶은면 두 순서 바꾸던지 -(this.W - o.W)
...
//		});
		...
		Arrays.sort(edges); //implements class에 추가 필요함
		
		//----------------------------===========
		//크루스칼 제 2장: V-1개의 간선을 뽑아라! (사이클이 발생하지 않게)
		//상호배타집합 (서로소집합, 유니온파인드)
		p = new int[V]; //0번부터 시작하길래 v개로 만들었따~
...
//			
			...
			if(pick == (V-1)) break; //총 뽑은 간선의 개수가 V-1라면 break;
...
//		return findSet(p[x]);
		...
		//아래는 패스 컴프레숀을 적용한 코드 //바로 부모로 직결시켜버림 //이거 안하면 깊게 갔다가 다시 올라와야 함
		if(x != p[x])
			p[x] = findSet(p[x]);
		return p[x];
	}
