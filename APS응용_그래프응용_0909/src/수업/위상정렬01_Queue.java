...
public class 위상정렬01_Queue {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<E; i++) {
			...
			adjArr[A][B] = 1; //가중치는 따로 없으니까 1로 표기 //유향
			degree[B]++;//진입 차수를 증가! //A->B이므로 B의 진입차수를 증가시킨다.
		}//입력 완료
		
		//위상정렬 큐 1장: 진입차수가 0인 친구들을 몽땅 넣어라!
		Queue<Integer> queue = new LinkedList<>();
		queue.add(9);//밥하기 먼저 나오게끔 해보았다.
		
		for(int i=1; i<=V; i++) {//1번부터 V번까지 [V+1]
			if(degree[i] == 0) {
				queue.add(i);//진입차수가 0이 된 애들을 큐에 넣어준다. 
			}
		}
		...
		while(!queue.isEmpty()) {
			...
			for(int i=1;i<=V;i++) { //진입차수[] 를 순회한다. 
				//무향에선 인덱스의 순서가 중요치 않아!
				//유향에선 인덱스의 순서가 중요하다
				if(adjArr[curr][i] == 1) {
					degree[i]--; //진입차수 하나 깎아.
					adjArr[curr][i] = 0; //이거 사실 1에서 깎는거니까 안해도 ㄱㅊ (근데 해야 하는 문제 있을 수 있음)
		...
		//밑에서 한방에 출력을 하고 싶다! (위에건 하나하나 꺼내는 방식이었다)
		//1. 여기서 꺼내진 이 친구를 list, 배열에 정점을 저장 (반복문 돌면서 매칭 시키며 출력)
		//2. StringBuilder로 쌓아놓는 방식이다. 
